...
void arena_init(void *start, void *end)
{
    ...
    gArena.head = start;
    for (i = 0; i < 4; i++)
        gArena.unk10[i] = gArena.startAddr;
}
...
void *arena_allocate(u32 size)
{
    ...
    gArena.head += (size + 3) & ~3;  // round up to nearest multiple of 4
    if (gArena.head > gArena.endAddr)
        print_message("Heap overflow.\nWe are out of RAM!");
    return block;
}

void arena_save_head(s32 index)
{
    gArena.unk10[index] = gArena.head;
}
...
void arena_restore_head(s32 index)
{
    if (index == -1)
        ...
        gArena.head = gArena.unk10[index];
}
